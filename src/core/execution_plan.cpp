...
struct ExecutionPlan::SemanticSeal {
  explicit SemanticSeal(const ExecutionPlan &plan)
      : tasks(plan.task_graph.tasks()),
        ...
        task_effects(plan.task_effects), task_facet_uses(plan.task_facet_uses),
        ...
        tier2_selection_requested(plan.tier2_selection_requested),
        tier2_selection_nodes(plan.tier2_selection_nodes),
...
namespace {
bool validate_representation_requests(
    const std::vector<RepresentationRequest> &requests, std::string *error) {
  for (size_t index = 0; index < requests.size(); ++index) {
    ...
    if (!request.view.valid(&view_error)) {
      if (error)
        *error = label +
                 " names a CanonicalView that cannot describe a Region: " +
                 view_error;
      return false;
    }
    if (request.target_kind == FacetKind::Address ||
        request.target_kind == FacetKind::Transfer) {
      if (error)
        *error =
            "representation request targets an AddressFacet or TransferFacet, "
            "which name how an existing "
            "representation is reached rather than a representation a "
...
bool freeze_representation_plan(
    const std::vector<RepresentationRequest> &requests, const Arena &arena,
  ...
  if (pool == nullptr) {
    if (error)
      *error = "representation planning requires the device FacetPool snapshot";
    return false;
  }
  if (!validate_representation_requests(requests, error))
    return false;
  out->clear();
  out->reserve(requests.size());
  for (uint32_t index = 0; index < requests.size(); ++index) {
    const auto &request = requests[index];
    if (!validate_facet_target(arena, request.view, request.target_kind, error))
      return false;
    const Allocation *allocation = arena.lookup(PointerRef{
        request.view.allocation, request.view.allocation_generation});
    if (allocation == nullptr) {
      if (error)
        *error =
            "representation request names an inactive allocation generation";
      return false;
    }
    if (request.consume_input &&
        pool->references({request.view.allocation,
                          request.view.allocation_generation,
                          allocation->representation_epoch})) {
      if (error)
        *error = "representation request asks for ConsumeInput while a live "
                 "FacetRef names its source epoch";
      return false;
    }
    out->push_back({request.view, request.target_kind, request.consume_input,
                    request.consume_proof, allocation->representation_epoch,
                    allocation->representation_epoch + 1, index});
  }
  return true;
}
...
bool representation_plan_matches(const ExecutionPlan &plan,
                                 std::string *error) {
  if (plan.representation_requests.empty())
    return plan.representation_plan.empty() &&
           !plan.representation_plan_derived;
  if (!plan.representation_plan_derived ||
      plan.representation_plan.size() != plan.representation_requests.size()) {
    if (error)
      *error = "assembled execution plan is missing frozen representation "
               "semantic facts";
    return false;
  }
  for (size_t i = 0; i < plan.representation_requests.size(); ++i) {
    const auto &r = plan.representation_requests[i];
    const auto &s = plan.representation_plan[i];
    if (!same_view(s.view, r.view) || s.target_kind != r.target_kind ||
        !same_proof(s.consume_proof, r.consume_proof) ||
        s.consume_input != r.consume_input || s.transform_order != i ||
        s.target_representation_epoch != s.source_representation_epoch + 1 ||
        (s.consume_input && s.consume_proof.first_unmet() != nullptr)) {
      if (error)
        *error =
            "representation request and frozen semantic plan item disagree";
      return false;
    }
  }
  return true;
...
bool same_schedule(const ExecutionSchedule &left,
  ...
  for (size_t component = 0; component < left.components.size(); ++component) {
    ...
    if (lhs.tasks != rhs.tasks || lhs.waves.size() != rhs.waves.size())
      return false;
    for (size_t wave = 0; wave < lhs.waves.size(); ++wave)
      if (lhs.waves[wave].tasks != rhs.waves[wave].tasks)
        return false;
  }
  for (size_t index = 0; index < left.transitions.size(); ++index) {
    ...
    if (lhs.component != rhs.component || lhs.before_wave != rhs.before_wave ||
        ...
        lhs.requires_execution_completion !=
            rhs.requires_execution_completion ||
      ...
      return false;
    for (size_t visibility = 0; visibility < lhs.region_visibility.size();
         ++visibility) {
      const auto &a = lhs.region_visibility[visibility];
      const auto &b = rhs.region_visibility[visibility];
      if (a.producer_task != b.producer_task ||
          a.consumer_task != b.consumer_task || a.allocation != b.allocation ||
          a.offset != b.offset || a.size != b.size ||
          a.representation_epoch != b.representation_epoch ||
          a.producer_access != b.producer_access ||
          a.consumer_access != b.consumer_access)
        return false;
    }
    for (size_t facet = 0; facet < lhs.facet_requirements.size(); ++facet) {
      ...
      if (a.task != b.task || !same_task_facet_use(a.use, b.use))
        return false;
    }
  }
  return true;
...
bool same_task_record(const TaskRecord &left, const TaskRecord &right) {
  return left.node_index == right.node_index &&
         ...
         same_facet_ref(left.raster_facets.source,
                        right.raster_facets.source) &&
         same_facet_ref(left.raster_facets.target,
                        right.raster_facets.target) &&
         same_facet_ref(left.depth_attachment_ref,
                        right.depth_attachment_ref) &&
...
bool contains(const std::vector<T> &values, const U &value, Equal equal) {
  return std::ranges::any_of(
      values, [&](const T &candidate) { return equal(candidate, value); });
}

bool append_touched(PointerRef ref, const Arena &arena,
                    std::vector<PointerRef> *out, std::string *error) {
  if (ref.allocation == 0 || ref.generation == 0 ||
      arena.lookup(ref) == nullptr) {
    if (error)
      *error = "task/static effect names an inactive allocation";
    return false;
  }
  if (!contains(*out, ref, same_ref))
    out->push_back(ref);
  return true;
}

bool append_lifetime_facet(FacetRef ref, FacetKind kind,
                           std::vector<FacetLifetimeUse> *out,
                           std::string *error) {
  ...
  if (ref.index == 0 && ref.generation == 0)
    return true;
  const auto found =
      std::ranges::find_if(*out, [&](const FacetLifetimeUse &use) {
        return same_facet_ref(use.ref, ref);
      });
  if (found != out->end()) {
    if (found->kind != kind) {
      if (error)
        *error = "one raster FacetRef is used with incompatible facet kinds";
      return false;
    }
    return true;
  }
...
bool facet_lifetime_less(const FacetLifetimeUse &left,
                         const FacetLifetimeUse &right) {
...
bool same_ref_sets(const std::vector<PointerRef> &left,
                   const std::vector<PointerRef> &right) {
  return std::ranges::all_of(
             left,
             [&](PointerRef ref) { return contains(right, ref, same_ref); }) &&
         std::ranges::all_of(right, [&](PointerRef ref) {
           return contains(left, ref, same_ref);
         });
}

bool sum_references(const Arena &arena,
                    const std::vector<PointerRef> &references, uint64_t *out,
                    std::string *error) {
  uint64_t total = 0;
  for (const PointerRef ref : references) {
    ...
    if (allocation == nullptr) {
      if (error)
        *error = "access plan names an inactive allocation";
      return false;
    }
...
bool build_complete_lease(const Arena &arena,
                          const std::vector<PointerRef> &proven,
                          WorkingSetLease *out, std::string *error) {
  WorkingSetLease lease;
  for (const PointerRef ref : proven) {
    if (!lease.add(ref, proven, error))
      return false;
  }
  if (!sum_references(arena, lease.allocations, &lease.byte_limit, error))
    return false;
  lease.complete = true;
  *out = std::move(lease);
  return true;
...
bool sum_active_bytes(const Arena &arena, uint64_t *out) {
  ...
  for (const auto &[id, allocation] : arena.allocations()) {
    (void)id;
    if (allocation.state == ObjectState::Active)
      total += allocation.size;
  }
  *out = total;
  return true;
...
bool append_bounded_pointer_targets(const ir::Module &module,
                                    const Arena &arena,
                                    std::vector<PointerRef> *out,
                                    std::string *error) {
  for (const auto &instruction : module.instructions) {
    if (instruction.op != "load_via" && instruction.op != "store_via")
      continue;
    if (!append_touched({instruction.allocation, instruction.generation}, arena,
                        out, error))
      return false;
  }
  return true;
...
std::vector<ir::Effect> instantiate_node_effects(const ir::Module &module) {
  ...
  for (const auto &instruction : module.instructions) {
    if (instruction.op != "load_via" && instruction.op != "store_via")
      continue;
    effects.push_back(
        {instruction.allocation, instruction.offset, instruction.size,
...
bool append_raster_facet_effect(
    const Arena &arena, const FacetPool *pool, FacetRef ref,
    FacetKind expected_kind, ir::Access access, RasterEffectRange range,
    uint32_t element_count, const char *label, std::vector<ir::Effect> *effects,
    std::vector<PointerRef> *touched, std::vector<FacetLifetimeUse> *lifetime,
    std::vector<TaskFacetSemanticUse> *task_facets, std::string *error) {
  if (absent(ref))
    return true;
  if (pool == nullptr) {
    if (error)
      *error = std::string("raster semantic assembly requires the submitting "
                           "FacetPool to resolve ") +
               label;
    return false;
  }
  ...
  if (slot == nullptr) {
    if (error)
      *error = std::string(label) + ": " + to_string(status);
    return false;
  }
  if (slot->kind != expected_kind) {
    if (error)
      *error = std::string(label) + ": facet kind mismatch";
    return false;
  }
  if (!append_touched({slot->view.allocation, slot->view.allocation_generation},
    ...
    return false;
  const Allocation *allocation = arena.lookup(
      PointerRef{slot->view.allocation, slot->view.allocation_generation});
  if (allocation == nullptr) {
    if (error)
      *error = std::string(label) + ": backing allocation is inactive";
    return false;
  }
  ...
  if (range == RasterEffectRange::FullBacking) {
    ...
    const uint64_t stride = slot->view.format == PixelFormat::R16Uint   ? 2u
                            : slot->view.format == PixelFormat::R32Uint ? 4u
                                                                        : 0u;
    if (stride == 0 || element_count % 3 != 0 ||
        element_count > UINT64_MAX / stride) {
      if (error)
        *error = "raster index facet requires R16Uint/R32Uint and a "
                 "triangle-list count";
      return false;
    }
  ...
  if (size == 0) {
    if (error)
      *error = std::string(label) + ": facet view has an empty effect range";
    return false;
  }
  if (size > allocation->bytes.size()) {
    if (error)
      *error = std::string(label) + ": effect range exceeds backing allocation";
    return false;
  }
  effects->push_back(
      {slot->view.allocation, 0, size, access, slot->representation_epoch});
  task_facets->push_back(
      {ref, expected_kind, slot->view, slot->representation_epoch, access});
  return true;
}

bool instantiate_raster_task_effects(
    const TaskRecord &task, std::string_view root_schema, const Arena &arena,
    const FacetPool *pool, std::vector<ir::Effect> *effects,
    std::vector<PointerRef> *touched, std::vector<FacetLifetimeUse> *lifetime,
    std::vector<TaskFacetSemanticUse> *task_facets, std::string *error) {
  effects->clear();
  task_facets->clear();
  ...
  if (is_scene_root_raster_schema(root_schema)) {
    ...
    if (!resolve_scene_root_raster(arena, task, &root, error))
      return false;
    source = root.albedo;
    effects->push_back({task.root_allocation, 0,
                        VG_SCHEMA_SCENEROOTRASTER_ROOT_SIZE, ir::Access::Read,
                        root.allocation->representation_epoch});
  }
  if (absent(source)) {
    if (error)
      *error = "raster source facet is required";
    return false;
  }
  if (absent(task.raster_facets.target)) {
    if (error)
      *error = "raster target facet is required";
    return false;
  }
  if (absent(task.vertex_buffer_ref)) {
    if (error)
      *error = "raster vertex facet is required";
    return false;
  }
  if (task.index_count != 0 && absent(task.index_buffer_ref)) {
    if (error)
      *error = "raster index facet is required when index_count is non-zero";
    return false;
  }
  if (absent(task.depth_attachment_ref) &&
      (task.depth_test_enable || task.depth_write_enable)) {
    if (error)
      *error = "raster depth state requires a depth attachment facet";
    return false;
  }
  if (!append_raster_facet_effect(arena, pool, source, FacetKind::Sample,
                                  ir::Access::Read, RasterEffectRange::View, 0,
                                  "raster source facet", effects, touched,
                                  lifetime, task_facets, error) ||
      !append_raster_facet_effect(
          arena, pool, task.raster_facets.target, FacetKind::Attachment,
          ir::Access::Write, RasterEffectRange::View, 0, "raster target facet",
  ...
  if (task.index_count != 0 &&
      !append_raster_facet_effect(
          arena, pool, task.index_buffer_ref, FacetKind::Address,
          ir::Access::Read, RasterEffectRange::IndexElements, task.index_count,
          "raster index facet", effects, touched, lifetime, task_facets, error))
    return false;
  if (!absent(task.depth_attachment_ref)) {
    if (task.depth_test_enable &&
        !append_raster_facet_effect(
            arena, pool, task.depth_attachment_ref, FacetKind::Attachment,
            ir::Access::Read, RasterEffectRange::View, 0, "raster depth facet",
            effects, touched, lifetime, task_facets, error))
      return false;
    ...
    if (!append_raster_facet_effect(
            arena, pool, task.depth_attachment_ref, FacetKind::Attachment,
            ir::Access::Write, RasterEffectRange::View, 0, "raster depth facet",
            effects, touched, lifetime, task_facets, error))
      return false;
  }
  return true;
...
bool envelope_authorizes(const ExecutionEnvelope &envelope,
                         NodeTable::Ref ref) {
  return envelope.allowed_nodes.empty() ||
         contains(envelope.allowed_nodes, ref, same_node_ref);
}

bool require_raster_only(const TaskGraph &graph, std::string_view reason,
                         std::string *error) {
...
void add_requirement(std::vector<CapabilityRequirement> *requirements,
                     CapabilityRequirement requirement) {
  if (!contains(*requirements, requirement,
                [](CapabilityRequirement left, CapabilityRequirement right) {
                  return left == right;
                }))
...
bool derive_capability_requirements(ExecutionPlan *plan, std::string *error) {
  ...
  if (!plan->task_graph.tasks().empty())
    add_requirement(&requirements, CapabilityRequirement::TaskPublication);
  ...
  if (!plan->representation_requests.empty())
    add_requirement(&requirements,
                    CapabilityRequirement::RepresentationTransform);
  if (plan->tier2_selection_requested)
    add_requirement(&requirements, CapabilityRequirement::IndirectTier2Select);
  ...
  for (const auto &task : plan->task_graph.tasks()) {
    ...
    case TaskKind::Raster:
      ...
      if (task.index_count != 0)
        add_requirement(&requirements, CapabilityRequirement::IndexedBinding);
      break;
    default:
      if (error)
        *error = "task execution domain is Unsupported by the canonical core "
                 "assembler";
      return false;
    }
  }
  for (const auto &node : plan->resolved_nodes) {
    if (node.user_raster_shader.has_value())
      add_requirement(&requirements, CapabilityRequirement::UserShaderImport);
    if (!node.module.has_value() && !node.user_raster_shader.has_value()) {
      if (error)
        *error = "resolved node format/domain is Unsupported by the canonical "
                 "core assembler";
      return false;
    }
  }
  ...
  case ValidationProfile::CheckedNative:
    add_requirement(&requirements,
                    CapabilityRequirement::CheckedFacetGeneration);
    break;
  ...
  default:
    if (error)
      *error = "unknown validation profile is unsupported";
    return false;
  }
...
bool build_validated_effect_graph(
    const TaskGraph &graph, const std::vector<std::vector<ir::Effect>> &effects,
    EffectGraph *out, std::string *error) {
  if (out == nullptr) {
    if (error)
      *error = "validated EffectGraph output is required";
    return false;
  }
  ...
  for (uint32_t left = 0; left < count; ++left) {
    for (uint32_t right = left + 1; right < count; ++right) {
      bool conflict = false;
      for (const auto &lhs : effects[left])
        for (const auto &rhs : effects[right])
          conflict = conflict || EffectGraph::conflicts(lhs, rhs);
      if (!conflict || reaches(left, right) || reaches(right, left))
        continue;
      if (!sealed.add_edge(left, right, EffectEdgeKind::InferredConflict, 0,
                           error))
        return false;
  ...
  if (!sealed.valid() || !sealed.validate_happens_before(effects, error)) {
    if (error && error->empty())
      *error = "assembler could not seal the validated EffectGraph";
    return false;
  }
  *out = std::move(sealed);
  return true;
...
const char *capability_requirement_name(CapabilityRequirement requirement) {
  switch (requirement) {
  case CapabilityRequirement::LinearAddress:
    return "LinearAddress";
  case CapabilityRequirement::TaskPublication:
    return "TaskPublication";
  case CapabilityRequirement::Timeline:
    return "Timeline";
  case CapabilityRequirement::EffectDag:
    return "EffectDag";
  case CapabilityRequirement::CaptureReplay:
    return "CaptureReplay";
  case CapabilityRequirement::IndirectTier1:
    return "IndirectTier1";
  case CapabilityRequirement::IndirectTier2Select:
    return "IndirectTier2Select";
  case CapabilityRequirement::IndexedBinding:
    return "IndexedBinding";
  case CapabilityRequirement::Raster:
    return "Raster";
  case CapabilityRequirement::RepresentationTransform:
    return "RepresentationTransform";
  case CapabilityRequirement::CheckedFacetGeneration:
    return "CheckedFacetGeneration";
  case CapabilityRequirement::UserShaderImport:
    return "UserShaderImport";
  case CapabilityRequirement::ReferenceStrict:
...
bool ExecutionPlan::validate(std::string *error) const {
  if (assembled) {
    if (!representation_plan_matches(*this, error))
      return false;
    if (!lifetime_plan_derived) {
      if (error)
        *error = "assembled execution plan is missing sealed lifetime facts";
      return false;
    }
    if (!std::is_sorted(lifetime_facets.begin(), lifetime_facets.end(),
                        facet_lifetime_less) ||
        std::adjacent_find(
            lifetime_facets.begin(), lifetime_facets.end(),
            [](const FacetLifetimeUse &left, const FacetLifetimeUse &right) {
              return same_facet_ref(left.ref, right.ref);
            }) != lifetime_facets.end()) {
      if (error)
        *error = "assembled execution plan facet lifetime facts are not "
                 "deterministic";
      return false;
    }
    if (!capability_requirements_derived) {
      if (error)
        *error = "assembled execution plan is missing sealed capability "
                 "requirements";
      return false;
    }
    if (!std::is_sorted(
        ...
        std::adjacent_find(required_capabilities.begin(),
                           required_capabilities.end()) !=
            required_capabilities.end()) {
      if (error)
        *error = "assembled execution plan capability requirements are not "
                 "deterministic";
      return false;
    }
    ...
    if (!derive_capability_requirements(&derived_requirements,
                                        &requirement_error) ||
        derived_requirements.required_capabilities != required_capabilities) {
      if (error)
        *error = requirement_error.empty()
                     ? "sealed capability requirements disagree with the "
                       "execution semantics"
                     : requirement_error;
      return false;
    }
    if (task_order.size() != task_graph.tasks().size() ||
      ...
      if (error)
        *error =
            "assembled execution plan is missing its deterministic task order";
      return false;
    }
    if (!execution_schedule_derived) {
      if (error)
        *error =
            "assembled execution plan is missing its sealed execution schedule";
      return false;
    }
    ...
    for (uint32_t task_index : task_order) {
      if (task_index >= task_effects.size() || seen_task[task_index] != 0) {
        if (error)
          *error = "assembled execution plan task order is out of range or "
    ...
    if (!validated_effect_graph.valid() ||
      ...
      if (error && error->empty())
        *error =
            "assembled execution plan is missing its validated EffectGraph";
      return false;
    }
    if (validated_effect_graph_shape !=
        classify_effect_graph_shape(
            validated_effect_graph,
            static_cast<uint32_t>(task_effects.size()))) {
      if (error)
        *error =
            "validated EffectGraph shape was tampered after semantic assembly";
      return false;
    }
    for (const auto &task : task_graph.tasks()) {
      const NodeTable::Ref ref{task.node_index, task.node_generation};
      if (std::count_if(resolved_nodes.begin(), resolved_nodes.end(),
                        [&](const ResolvedNode &node) {
                          return same_node_ref(node.ref, ref);
                        }) != 1) {
        if (error)
          *error = "assembled execution plan has a duplicate or missing "
                   "resolved task node";
        return false;
      }
    }
    ...
    if (resolved_nodes.size() != unique_task_refs.size()) {
      if (error)
        *error =
            "assembled execution plan contains a resolved Node with no Task";
      return false;
    }
    for (const auto &node : resolved_nodes) {
      if (!node.code_object ||
          ...
          node.user_raster_shader.has_value() !=
              node.code_object->user_raster_shader.has_value()) {
        if (error)
          *error = "resolved Node program disagrees with its immutable "
                   "CodeObject snapshot";
        return false;
      }
      if (node.module.has_value()) {
        const std::string resolved_canonical =
            ir::serialize_module(*node.module);
        const std::string object_canonical =
            ir::serialize_module(*node.code_object->module);
        if (resolved_canonical != object_canonical ||
            node.module->canonical_json !=
                node.code_object->module->canonical_json ||
            node.module->hash != node.code_object->module->hash) {
          if (error)
            *error = "resolved Node module drifted from its immutable "
                     "CodeObject snapshot";
          return false;
        }
      }
      if (node.user_raster_shader.has_value()) {
        ...
        if (resolved.root_schema != object.root_schema ||
          ...
          if (error)
            *error = "resolved raster contract drifted from its immutable "
                     "CodeObject snapshot";
          return false;
        }
      }
      ...
      for (const auto &task : task_graph.tasks()) {
        if (!same_node_ref(node.ref, NodeTable::Ref{task.node_index,
        ...
        if (!observed_domain) {
          ...
          if (error)
            *error =
                "one resolved NodeRef is used by multiple execution domains";
          return false;
        }
      }
      if (!observed_domain || node.execution_domain != expected_domain ||
        ...
        if (error)
          *error = "resolved Node execution domain disagrees with its Tasks or "
                   "contract";
        return false;
      }
    }
    for (size_t task_index = 0; task_index < task_graph.tasks().size();
         ++task_index) {
      const auto &task = task_graph.tasks()[task_index];
      const NodeTable::Ref ref{task.node_index, task.node_generation};
      const auto resolved =
          std::ranges::find_if(resolved_nodes, [&](const ResolvedNode &node) {
            return same_node_ref(node.ref, ref);
          });
      if (resolved == resolved_nodes.end()) {
        if (error)
          *error = "assembled execution plan is missing its immutable per-Task "
                   "Node snapshot";
        return false;
      }
      if (task.kind == TaskKind::Compute) {
        if (!task_facet_uses[task_index].empty()) {
          if (error)
            *error = "compute Task contains sealed raster facet semantics";
          return false;
        }
        const auto expected = resolved->module.has_value()
                                  ? instantiate_node_effects(*resolved->module)
                                  : std::vector<ir::Effect>{};
        if (expected.size() != task_effects[task_index].size() ||
            !std::equal(expected.begin(), expected.end(),
                        task_effects[task_index].begin(), same_effect)) {
          if (error)
            *error = "sealed per-Task effects disagree with the immutable Node "
                     "program";
          return false;
        }
      } else {
        if (task_facet_uses[task_index].empty()) {
          if (error)
            *error = "raster Task is missing sealed per-Task facet semantics";
          return false;
        }
        for (const auto &use : task_facet_uses[task_index]) {
          ...
          if (absent(use.ref) || !use.view.valid(&view_error) ||
              !std::ranges::any_of(
                  task_effects[task_index], [&](const ir::Effect &effect) {
                    return effect.allocation == use.view.allocation &&
                           effect.representation_epoch ==
                               use.representation_epoch &&
                           effect.access == use.access;
                  })) {
            if (error)
              *error = "sealed raster facet semantics disagree with per-Task "
                       "effects";
            return false;
          }
        }
    ...
    if (!build_validated_effect_graph(task_graph, task_effects,
        ...
        expected_effect_graph.edges().size() !=
            validated_effect_graph.edges().size() ||
      ...
      if (error)
        *error = graph_error.empty() ? "validated EffectGraph disagrees with "
                                       "canonical TaskGraph/effect derivation"
                                     : graph_error;
      return false;
    }
    std::vector<ir::Effect> actual_flattened;
    for (uint32_t task_index : task_order)
      actual_flattened.insert(actual_flattened.end(),
                              task_effects[task_index].begin(),
    ...
    if (actual_flattened.size() != instantiated_effects.size() ||
      ...
      if (error)
        *error = "flattened effect certificate facts disagree with the "
                 "per-Task facts";
      return false;
    }
    std::vector<NodeTable::Ref> resolved_node_refs;
    ...
    if (!semantic_seal ||
        ...
        !same_node_refs(resolved_node_refs,
                        semantic_seal->resolved_node_refs) ||
        resolved_node_domains != semantic_seal->resolved_node_domains ||
        !same_task_effects(task_effects, semantic_seal->task_effects) ||
        !same_task_facet_uses(task_facet_uses,
                              semantic_seal->task_facet_uses) ||
        !same_effects(instantiated_effects,
                      semantic_seal->instantiated_effects) ||
        !same_effect_edges(validated_effect_graph.edges(),
                           semantic_seal->effect_edges) ||
        ...
        tier2_selection_requested != semantic_seal->tier2_selection_requested ||
        !same_node_refs(tier2_selection_nodes,
                        semantic_seal->tier2_selection_nodes) ||
        !same_effects(certificate.ranges, semantic_seal->certificate_ranges) ||
        !same_pointer_ref_list(touched_allocations,
                               semantic_seal->touched_allocations) ||
        !same_lifetime_uses(lifetime_facets, semantic_seal->lifetime_facets)) {
      if (error)
        *error = "assembled execution plan semantic facts disagree with the "
                 "immutable assembler seal";
      return false;
    }
    ExecutionSchedule expected_schedule;
    const auto representation_facts =
        schedule_representation_facts(representation_plan);
    ...
    if (!derive_execution_schedule(validated_effect_graph, task_effects,
      ...
      if (error)
        *error = schedule_error.empty() ? "sealed execution schedule disagrees "
                                          "with Stage-5 semantic facts"
                                        : schedule_error;
      return false;
    }
    std::vector<uint32_t> effect_order;
    if (!effect_graph_deterministic_order(
            validated_effect_graph, static_cast<uint32_t>(task_effects.size()),
            &effect_order, error) ||
        effect_order != task_order) {
      if (error && error->empty())
        *error = "validated EffectGraph order disagrees with the sealed "
                 "TaskGraph order";
      return false;
    }
    if (requested_certificate_mode.has_value()) {
      if (!access_plan_derived || !access_certificate.has_value()) {
        if (error)
          *error = "assembled execution plan is missing sealed access-planning "
                   "facts";
        return false;
      }
      if (access_certificate->mode != *requested_certificate_mode) {
        if (error)
          *error = "assembled access certificate mode does not match the "
                   "execution envelope";
        return false;
      }
      if (!certificate_covers_discovery_witness(*access_certificate,
                                                touched_allocations, error))
        return false;
      if (discovery_result.has_value()) {
        if (discovery_result->frozen_topology_epoch != graph_epoch ||
            !same_ref_sets(discovery_result->reachable,
                           access_certificate->epoch.references())) {
          if (error)
            *error = "assembled discovery witness and access certificate are "
                     "inconsistent";
          return false;
        }
      }
  ...
  if (assembled) {
    for (const auto &node : resolved_nodes) {
      if (node.user_raster_shader.has_value()) {
        if (node.user_raster_shader->vertex_abi !=
            ir::kRasterVertexAbiXyzuvPackedV1) {
          if (error)
            *error = "a resolved user_raster_shader requires vertex_abi "
                     "vg.raster.vertex.xyzuv-packed/v1";
          return false;
        }
        ...
        if (is_scene_root_raster_schema(node.module->root_schema) &&
            !require_raster_only(
                task_graph,
                "a SceneRoot raster submission may only contain raster tasks; "
                "compute+raster mixing is deferred",
                error))
          return false;
      } else {
        if (error)
          *error = "assembled execution plan has a resolved node without a "
                   "runnable program";
        return false;
      }
    }
    ...
    if (error)
      *error =
          "execution plan must be produced by the canonical core assembler";
    return false;
  }
  if (timeline_signal != 0 && timeline_signal <= timeline_wait) {
    if (error)
      *error = "timeline signal does not advance past wait";
    return false;
  }
  if (!published && !task_graph.tasks().empty()) {
    if (error)
      *error = "execution plan contains unpublished tasks";
    return false;
  }
  if (published && !task_graph.tasks().empty() &&
      !task_graph.validate_execution(error))
    return false;
  if (!validate_representation_requests(representation_requests, error))
    return false;
  if (working_set_budget && working_set_lease &&
      working_set_budget->has_limit &&
      working_set_lease->byte_limit > working_set_budget->byte_limit) {
    if (error)
      *error = "working-set lease exceeds the plan's working-set budget";
    return false;
  }
  if (pending_overflow && !pending_overflow->valid(error))
    return false;
  return true;
}

bool ExecutionPlan::graph_epoch_matches(const Arena &arena,
                                        std::string *error) const {
  if (task_graph.tasks().empty())
    return true;
  if (graph_epoch != arena.topology_epoch()) {
    if (error)
      *error = "execution plan graph epoch does not match arena topology";
    return false;
  }
  return true;
}

bool ExecutionPlanAssembler::assemble(
    const ExecutionPlanAssemblerInputs &inputs, ExecutionPlan *out,
    std::string *error) {
  if (out == nullptr || inputs.task_graph == nullptr ||
      inputs.nodes == nullptr || inputs.envelope == nullptr ||
      inputs.arena == nullptr) {
    if (error)
      *error = "execution-plan assembly requires graph, node table, envelope, "
               "arena, and output";
    return false;
  }
  ...
  if (!graph.sealed() || !graph.published()) {
    if (error)
      *error =
          "execution-plan assembly requires a sealed and published task graph";
    return false;
  }
  if (graph.tasks().empty()) {
    if (error)
      *error = "execution-plan assembly requires at least one task";
    return false;
  }
  if (!graph.validate_execution(error))
    return false;
  ...
  plan.published = true;
  plan.graph_epoch =
      inputs.graph_epoch == 0 ? arena.topology_epoch() : inputs.graph_epoch;
  plan.authorized_nodes = envelope.allowed_nodes;
  if (inputs.tier2_selection_nodes != nullptr) {
    plan.tier2_selection_requested = true;
    plan.tier2_selection_nodes = *inputs.tier2_selection_nodes;
    if (plan.tier2_selection_nodes.size() < 2) {
      if (error)
        *error = "Tier2 selection request requires at least two complete "
                 "preauthorized NodeRefs";
      return false;
    }
    for (size_t i = 0; i < plan.tier2_selection_nodes.size(); ++i) {
      const auto ref = plan.tier2_selection_nodes[i];
      NodeEntry snapshot;
      const bool in_envelope = std::ranges::any_of(
          envelope.allowed_nodes, [ref](NodeTable::Ref candidate) {
            return candidate.index == ref.index &&
                   candidate.generation == ref.generation;
          });
      if (ref.generation == 0 || !in_envelope ||
          !inputs.nodes->snapshot(ref, &snapshot) || !snapshot.code_object) {
        if (error)
          *error = "Tier2 selection request contains a stale or "
                   "envelope-unauthorized NodeRef";
        return false;
      }
      for (size_t prior = 0; prior < i; ++prior)
        if (plan.tier2_selection_nodes[prior].index == ref.index &&
            plan.tier2_selection_nodes[prior].generation == ref.generation) {
          if (error)
            *error = "Tier2 selection request contains duplicate NodeRefs";
          return false;
        }
    }
  }
  ...
  if (inputs.representation_requests != nullptr)
    plan.representation_requests = *inputs.representation_requests;
  if (!freeze_representation_plan(plan.representation_requests, arena,
                                  inputs.facet_pool, &plan.representation_plan,
                                  error))
    return false;
  plan.representation_plan_derived = !plan.representation_requests.empty();
  if (inputs.discovery_seeds != nullptr)
    plan.discovery_seeds = *inputs.discovery_seeds;
  if (inputs.working_set_budget != nullptr)
    plan.working_set_budget = *inputs.working_set_budget;
  if (inputs.working_set_lease != nullptr)
    plan.working_set_lease = *inputs.working_set_lease;
  if (inputs.pending_overflow != nullptr)
    plan.pending_overflow = *inputs.pending_overflow;
  if (envelope.has_task_quota)
    plan.envelope_task_quota = envelope.task_quota;
  if (envelope.has_certificate_mode)
    plan.requested_certificate_mode = envelope.certificate_mode;
  if (plan.graph_epoch != arena.topology_epoch()) {
    if (error)
      *error =
          "execution-plan assembly graph epoch does not match arena topology";
    return false;
  }
  ...
  for (const auto &touched : envelope.certificate_touched) {
    ...
    if (!append_touched(touched, arena, &checked, error)) {
      if (error)
        *error = "execution envelope contains an invalid touched allocation";
      return false;
    }
  }
  ...
  for (const auto &item : plan.representation_plan) {
    if (!append_touched({item.view.allocation, item.view.allocation_generation},
                        arena, &actual_touched, error)) {
      if (error)
        *error = "representation plan names an inactive allocation generation";
      return false;
    }
  }
  plan.task_effects.resize(graph.tasks().size());
  plan.task_facet_uses.resize(graph.tasks().size());
  for (uint32_t task_index = 0; task_index < graph.tasks().size();
       ++task_index) {
    ...
    if (!inputs.nodes->snapshot(ref, &snapshot) || !snapshot.code_object) {
      if (error)
        *error = "task graph references an unknown or stale node";
      return false;
    }
    if (!envelope_authorizes(envelope, ref)) {
      if (error)
        *error = "execution envelope does not authorize task node generation";
      return false;
    }
    ...
    if (task.root_allocation != 0 || task.kind != TaskKind::Raster) {
      if (!append_touched({task.root_allocation, task.root_generation}, arena,
                          &actual_touched, error)) {
        if (error)
          *error = "task root cannot be bound to an active allocation";
        return false;
      }
    }
    if (!contains(
            plan.resolved_nodes, ref,
            [](const ExecutionPlan::ResolvedNode &node, NodeTable::Ref wanted) {
              return same_node_ref(node.ref, wanted);
            })) {
      const bool compute_node =
          snapshot.code_object->module.has_value() &&
          !snapshot.code_object->user_raster_shader.has_value();
      const bool raster_node =
          snapshot.code_object->user_raster_shader.has_value() &&
          !snapshot.code_object->module.has_value();
      if (!compute_node && !raster_node) {
        if (error)
          *error = "node execution domain is Unsupported by the canonical core "
                   "assembler";
        return false;
      }
      // Canonical IR remains usable for the built-in raster Task contract; only
      // a restricted imported raster package is raster-exclusive.
      if (raster_node && task.kind != TaskKind::Raster) {
        if (error)
          *error =
              "task kind does not match its resolved node execution domain";
        return false;
      }
      if (compute_node) {
        const ir::Module &module = *snapshot.code_object->module;
        const std::string canonical = ir::serialize_module(module);
        if (module.canonical_json != canonical ||
            module.hash != ir::sha256_hex(canonical)) {
          if (error)
            *error = "materialized module canonical hash does not match its "
                     "serialized module";
          return false;
        }
        ...
        if (!verified.ok) {
          if (error)
            *error = "materialized module failed canonical IR verification: " +
                     verified.message;
          return false;
        }
        ...
        if (!append_bounded_pointer_targets(module, arena, &actual_touched,
                                            error))
          return false;
        plan.resolved_nodes.push_back({ref, snapshot.code_object,
                                       snapshot.entry_name, module,
                                       std::nullopt, task.kind});
      } else {
        plan.resolved_nodes.push_back(
            {ref, snapshot.code_object, snapshot.entry_name, std::nullopt,
             snapshot.code_object->user_raster_shader, TaskKind::Raster});
      }
    }
    ...
    const auto resolved = std::find_if(
        plan.resolved_nodes.begin(), plan.resolved_nodes.end(),
        [&](const auto &node) { return same_node_ref(node.ref, ref); });
    std::vector<ir::Effect> effects;
    if (task.kind == TaskKind::Compute) {
      if (!resolved->module.has_value()) {
        if (error)
          *error = "compute Task does not resolve to a canonical module";
        return false;
      }
      effects = instantiate_node_effects(*resolved->module);
    } else {
      const std::string &root_schema =
          resolved->user_raster_shader.has_value()
              ? resolved->user_raster_shader->root_schema
              : resolved->module->root_schema;
      if (!instantiate_raster_task_effects(
              task, root_schema, arena, inputs.facet_pool, &effects,
              &actual_touched, &plan.lifetime_facets,
    ...
    if (task.kind == TaskKind::Compute)
      for (const auto &effect : effects)
        if (!append_touched({effect.allocation, 1}, arena, &actual_touched,
                            error))
          return false;
  }
  if (!build_validated_effect_graph(graph, plan.task_effects,
                                    &plan.validated_effect_graph, error))
    return false;
  if (!effect_graph_deterministic_order(
          plan.validated_effect_graph,
          static_cast<uint32_t>(plan.task_effects.size()), &plan.task_order,
          error))
    return false;
  const auto representation_facts =
  ...
  for (const auto &task : graph.tasks()) {
    const NodeTable::Ref ref{task.node_index, task.node_generation};
    if (std::count_if(plan.resolved_nodes.begin(), plan.resolved_nodes.end(),
                      [&](const auto &node) {
                        return same_node_ref(node.ref, ref);
                      }) != 1) {
      if (error)
        *error = "resolved-node snapshot has a duplicate or missing task node";
      return false;
    }
  }
  if (plan.resolved_nodes.empty()) {
    if (error)
      *error = "task graph contains no resolvable node program";
    return false;
  }
  if (inputs.certificate != nullptr) {
    if (!validate_certificate(*inputs.certificate, plan.instantiated_effects,
                              error))
      return false;
  ...
  if (envelope.has_certificate_mode) {
    if (envelope.certificate_mode == AccessCertificateMode::SoftwarePaged ||
      ...
      if (error)
        *error = "requested access-certificate mode is Unsupported by the "
                 "canonical core assembler";
      return false;
    }
    ...
    if (envelope.certificate_mode == AccessCertificateMode::CertifiedPinned &&
        !envelope_covers_actual()) {
      if (error)
        *error = "execution envelope certificate_touched does not authorize "
                 "every task access";
      return false;
    }
    ...
    if (envelope.certificate_mode == AccessCertificateMode::DiscoverThenLease) {
      if (plan.discovery_seeds.empty()) {
        if (error)
          *error = "DiscoverThenLease requires non-empty seed topology for a "
                   "complete discovery witness";
        return false;
      }
      DiscoveryResult discovery;
      if (!discover_reachable(arena, plan.discovery_seeds, &discovery, error))
        return false;
      if (discovery.frozen_topology_epoch != plan.graph_epoch ||
          arena.topology_epoch() != plan.graph_epoch) {
        if (error)
          *error = "DiscoverThenLease discovery witness does not share the "
                   "execution-plan topology epoch";
        return false;
      }
      if (inputs.discovery_witness != nullptr &&
          !same_ref_sets(*inputs.discovery_witness, discovery.reachable)) {
        if (error)
          *error = "caller discovery witness disagrees with the "
                   "core-discovered reachable set";
        return false;
      }
      witness = std::move(discovery.reachable);
      if (!std::ranges::all_of(actual_touched, [&](PointerRef ref) {
            return contains(witness, ref, same_ref);
          })) {
        if (error)
          *error = "discovery witness does not cover every task access";
        return false;
      }
    ...
    if (access == nullptr) {
      if (envelope.certificate_mode ==
          AccessCertificateMode::DiscoverThenLease) {
        if (!build_discovered_certificate(arena, *plan.discovery_result,
                                          &generated, error))
          return false;
      } else if (!build_access_certificate(arena, envelope.certificate_mode,
                                           witness, &generated, error)) {
        return false;
      }
      access = &generated;
    }
    if (access->mode != envelope.certificate_mode ||
        access->epoch.value() != plan.graph_epoch ||
        !certificate_covers_discovery_witness(*access, witness, error)) {
      if (access->mode != envelope.certificate_mode && error)
        *error = "execution-plan assembly access certificate mode does not "
                 "match the envelope";
      else if (access->epoch.value() != plan.graph_epoch && error)
        *error = "execution-plan assembly access certificate does not share "
                 "the frozen topology epoch";
      return false;
    }
    ...
    if (envelope.certificate_mode == AccessCertificateMode::CertifiedPinned &&
      ...
      if (error)
        *error = "CertifiedPinned access certificate exceeds execution "
                 "envelope authority";
      return false;
    }
    ...
    const std::vector<PointerRef> &proven =
        plan.access_certificate->epoch.references();
    if (envelope.certificate_mode == AccessCertificateMode::DiscoverThenLease) {
      if (inputs.working_set_lease != nullptr) {
        if (!inputs.working_set_lease->valid(witness, error) ||
            !inputs.working_set_lease->complete ||
            !std::ranges::all_of(witness, [&](PointerRef ref) {
              return inputs.working_set_lease->covers(ref);
            })) {
          if (error && error->empty())
            *error = "DiscoverThenLease requires a complete lease covering its "
                     "witness";
          return false;
        }
      }
      WorkingSetLease discovered_lease;
      if (!build_complete_lease(arena, witness, &discovered_lease, error))
        return false;
    ...
    if (plan.working_set_lease.has_value()) {
      if (!sum_references(arena, plan.working_set_lease->allocations,
                          &plan.working_set_requested_bytes, error))
        return false;
    } else {
      ...
      if (!sum_active_bytes(arena, &plan.working_set_requested_bytes))
        return false;
    }
    if (plan.working_set_budget.has_value()) {
      std::string budget_error;
      if (!plan.working_set_budget->allows(plan.working_set_requested_bytes,
                                           &budget_error)) {
        if (error)
          *error = budget_error;
        return false;
  ...
  } else if (plan.working_set_budget.has_value() ||
             plan.working_set_lease.has_value() ||
             !plan.discovery_seeds.empty()) {
    if (error)
      *error = "working-set or discovery planning requires an explicit "
               "existing access-certificate mode";
    return false;
  }
  plan.touched_allocations = std::move(actual_touched);
  std::sort(plan.lifetime_facets.begin(), plan.lifetime_facets.end(),
            facet_lifetime_less);
  plan.lifetime_plan_derived = true;
  if (!derive_capability_requirements(&plan, error))
    return false;
  plan.assembled = true;
  plan.semantic_seal =
      std::make_shared<const ExecutionPlan::SemanticSeal>(plan);
