...
CompileResult compile_c_like(const std::string& source) {
  ...
  for(std::sregex_iterator it(source.begin(),source.end(),op),end;it!=end;++it){const auto& match=*it;ir::Instruction instruction;instruction.op=match[1];instruction.allocation=std::stoull(match[2]);instruction.offset=std::stoull(match[3]);instruction.size=std::stoull(match[4]);instruction.generation=1;if(match[5].matched)instruction.value=std::stoll(match[5]);module.instructions.push_back(instruction);ir::Access access=ir::access_from_op(instruction.op, ir::Access::Publish);module.declared_effects.push_back({instruction.allocation,instruction.offset,instruction.size,access,0});}
