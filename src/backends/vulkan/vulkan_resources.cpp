...
FacetUseGuard::FacetUseGuard(vg::core::FacetPool &pool, vg::core::FacetRef ref)
    : pool_(pool), ref_(ref) {}
FacetUseGuard::~FacetUseGuard() {
  if (held_)
    pool_.end_gpu_use(ref_);
}
...
bool create_raw_buffer(VkDevice device, VkPhysicalDevice physical_device,
  ...
  buffer_info.usage = usage |
      (want_address ? VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT : 0);
  buffer_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
  if (vkCreateBuffer(device, &buffer_info, nullptr, &out->buffer) !=
      VK_SUCCESS) {
    if (error)
  ...
  if (!find_memory_type(physical_device, requirements.memoryTypeBits,
    ...
    if (error)
      *error = "no host-visible-coherent memory type available";
    return false;
  }
  ...
  if (vkAllocateMemory(device, &alloc_info, nullptr, &out->memory) !=
      VK_SUCCESS) {
    destroy_raw_buffer(device, out);
    if (error)
  ...
  if (want_map && vkMapMemory(device, out->memory, 0, VK_WHOLE_SIZE, 0,
...
bool DeviceState::ensure_buffer(const vg::core::Allocation &allocation,
                                AllocationRecord **out, std::string *error) {
  const size_t needed = std::max<size_t>(allocation.bytes.size(), 1);
  auto it = allocation_map_.find(allocation.id);
  if (it != allocation_map_.end() &&
      (it->second.generation != allocation.generation ||
       it->second.byte_size < needed)) {
  ...
  if (it == allocation_map_.end()) {
    ...
    const VkDeviceAddress address =
        vkGetBufferDeviceAddress(device_, &address_info);

    AllocationRecord record{
        buffer, memory, address, mapped, allocation.generation, needed};
    it = allocation_map_.emplace(allocation.id, record).first;
  }
  if (!allocation.bytes.empty())
    std::memcpy(it->second.mapped, allocation.bytes.data(),
...
VkFormat to_vk_format(vg::core::PixelFormat format) {
  if (format == vg::core::PixelFormat::RGBA8Unorm)
    return VK_FORMAT_R8G8B8A8_UNORM;
  if (format == vg::core::PixelFormat::Depth32Float)
...
VkComponentSwizzle to_vk_swizzle(vg::core::Swizzle swizzle) {
  switch (swizzle) {
  case vg::core::Swizzle::Red:
    return VK_COMPONENT_SWIZZLE_R;
  case vg::core::Swizzle::Green:
    return VK_COMPONENT_SWIZZLE_G;
  case vg::core::Swizzle::Blue:
    return VK_COMPONENT_SWIZZLE_B;
  case vg::core::Swizzle::Alpha:
    return VK_COMPONENT_SWIZZLE_A;
  case vg::core::Swizzle::Zero:
    return VK_COMPONENT_SWIZZLE_ZERO;
  case vg::core::Swizzle::One:
...
uint32_t packed_swizzle(const vg::core::SwizzleChannels &swizzle) {
  return (static_cast<uint32_t>(swizzle.red) << 24) |
         (static_cast<uint32_t>(swizzle.green) << 16) |
         (static_cast<uint32_t>(swizzle.blue) << 8) |
         static_cast<uint32_t>(swizzle.alpha);
}

VkImageViewType to_vk_view_type(vg::core::ViewDimension dimension) {
  return dimension == vg::core::ViewDimension::Texture2DArray
...
VkImageLayout facet_read_layout(vg::core::FacetKind kind, VkFormat format) {
  switch (kind) {
  case vg::core::FacetKind::Sample:
    return VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
  case vg::core::FacetKind::Storage:
    return VK_IMAGE_LAYOUT_GENERAL;
  case vg::core::FacetKind::Attachment:
...
VkImageUsageFlags facet_image_usage(vg::core::FacetKind kind, VkFormat format) {
  ...
  if (kind == vg::core::FacetKind::Sample)
    usage |= VK_IMAGE_USAGE_SAMPLED_BIT;
  if (kind == vg::core::FacetKind::Storage)
    usage |= VK_IMAGE_USAGE_STORAGE_BIT;
  if (kind == vg::core::FacetKind::Attachment)
    usage |= format == VK_FORMAT_D32_SFLOAT
...
bool DeviceState::create_task_ring_buffers(uint32_t task_count,
                                           TaskRingBuffers *out,
                                           std::string *error) {
  static_assert(vg::compiler::kTaskRingDispatchXWord + 2 <
                vg::compiler::kTaskRingWordsPerRecord);
  const VkDeviceSize state_size =
      std::max<VkDeviceSize>(task_count * sizeof(uint32_t), sizeof(uint32_t));
  const VkDeviceSize record_size = std::max<VkDeviceSize>(
      static_cast<VkDeviceSize>(task_count) *
  ...
  RawBuffer state;
  if (!create_raw_buffer(device_, physical_device_, state_size, ring_usage,
                         /*want_address=*/true,
                         /*want_map=*/true, &state, error))
    return false;
  RawBuffer fields;
  if (!create_raw_buffer(device_, physical_device_, record_size, ring_usage,
                         true, true, &fields, error)) {
    destroy_raw_buffer(device_, &state);
    return false;
  }
  RawBuffer inputs;
  if (!create_raw_buffer(device_, physical_device_, record_size, ring_usage,
                         true, true, &inputs, error)) {
    destroy_raw_buffer(device_, &state);
...
void DeviceState::destroy_task_ring_buffers(TaskRingBuffers *buffers) {
  RawBuffer state{buffers->state_buffer, buffers->state_memory,
                  buffers->state_address, buffers->state_mapped};
  RawBuffer fields{buffers->fields_buffer, buffers->fields_memory,
                   buffers->fields_address, buffers->fields_mapped};
  RawBuffer inputs{buffers->inputs_buffer, buffers->inputs_memory,
                   buffers->inputs_address, buffers->inputs_mapped};
  destroy_raw_buffer(device_, &state);
  destroy_raw_buffer(device_, &fields);
...
const vg::core::FacetSlot *DeviceState::resolve_facet(
    const vg::core::Arena &arena, const vg::core::FacetPool &pool,
  ...
  if (slot == nullptr) {
    if (error)
      *error = std::string("facet reference did not resolve: ") +
               vg::core::to_string(status);
    return nullptr;
  }
  if (slot->kind != expected_kind) {
    if (error)
      *error = "facet kind mismatch: the pool slot names a different facet of "
               "this CanonicalView";
    return nullptr;
  }
  return slot;
...
bool DeviceState::ensure_facet_image(
    const vg::core::Arena &arena, const vg::core::FacetPool &pool,
    vg::core::FacetRef ref, vg::core::FacetKind expected_kind,
  ...
  if (cache_hit != nullptr)
    *cache_hit = false;
  if (temporary_bytes != nullptr)
    *temporary_bytes = 0;
  const vg::core::FacetSlot *slot =
      resolve_facet(arena, pool, ref, expected_kind, error);
  if (slot == nullptr)
    return false;
  const vg::core::CanonicalView view = slot->view;

  if (expected_kind != vg::core::FacetKind::Sample &&
      expected_kind != vg::core::FacetKind::Storage &&
      expected_kind != vg::core::FacetKind::Attachment) {
    if (error)
      *error = "Unsupported: AddressFacet/TransferFacet name how an existing "
               "linear representation is "
               "reached (BDA), not an image representation, so neither has a "
               "VkImage here (02 §3.3)";
    return false;
  }
  ...
  if (!view.swizzle.identity() &&
      expected_kind != vg::core::FacetKind::Sample) {
    if (error)
      *error = "Unsupported: a non-identity swizzle applies to a SampleFacet "
               "only; a Storage or "
  ...
  if (!support.transfer_dst || !support.transfer_src) {
    if (error)
      *error = "Unsupported: this device's optimal tiling does not advertise "
               "transfer for the view's "
               "format, so the linear backing cannot be copied into an image "
               "representation";
    return false;
  }
  if (expected_kind == vg::core::FacetKind::Sample && !support.sampled_image) {
    if (error)
      *error = "Unsupported: the view's format is not a sampled-image format "
               "on this device";
    return false;
  }
  if (expected_kind == vg::core::FacetKind::Storage && !support.storage_image) {
    if (error)
      *error = "Unsupported: the view's format is not a storage-image format "
               "on this device; the "
               "format is not rewritten to make the write legal (06 §6.2)";
    return false;
  }
  if (expected_kind == vg::core::FacetKind::Attachment) {
    const bool depth = view.format == vg::core::PixelFormat::Depth32Float;
    if ((depth && !support.depth_stencil_attachment) ||
        (!depth && !support.color_attachment)) {
      if (error)
        *error = depth
                     ? "Unsupported: Depth32Float is not a depth-stencil attachment format on this device"
                     : "Unsupported: the view's format is not a color-attachment format on this device";
      return false;
    }
  }
  ...
  const vg::core::Allocation *backing = arena.lookup(
      core::PointerRef{view.allocation, view.allocation_generation});
  // A cached image stays valid across ConsumeInput when its linear backing is
  // intentionally gone. While bytes remain, content_epoch prevents a compute
  // write or prior raster store from being hidden by a stale cached image.
  auto cached = facet_images_.find(key);
  if (cached != facet_images_.end() &&
      (backing == nullptr || backing->bytes.empty() ||
       cached->second.content_epoch == backing->content_epoch)) {
    if (cache_hit != nullptr)
      *cache_hit = true;
    *out = &cached->second;
    return true;
  ...
  if (cached != facet_images_.end()) {
    if (cached->second.view != VK_NULL_HANDLE)
      vkDestroyImageView(device_, cached->second.view, nullptr);
    if (cached->second.image != VK_NULL_HANDLE)
      vkDestroyImage(device_, cached->second.image, nullptr);
    if (cached->second.memory != VK_NULL_HANDLE)
      vkFreeMemory(device_, cached->second.memory, nullptr);
    facet_images_.erase(cached);
  }
  ...
  image_info.usage = facet_image_usage(expected_kind, format);
  image_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
  image_info.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;

  VulkanFacetRecord record{};
  if (vkCreateImage(device_, &image_info, nullptr, &record.image) !=
      VK_SUCCESS) {
    if (error)
      *error = "vkCreateImage failed for the facet's image representation";
    return false;
  }
  ...
  if (!find_memory_type(physical_device_, requirements.memoryTypeBits,
    ...
    destroy_partial();
    if (error)
      *error = "no device-local memory type available for an optimal-tiled "
               "facet image";
    return false;
  }
  VkMemoryAllocateInfo alloc_info{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};
  alloc_info.allocationSize = requirements.size;
  alloc_info.memoryTypeIndex = memory_type;
  if (vkAllocateMemory(device_, &alloc_info, nullptr, &record.memory) !=
      VK_SUCCESS) {
    destroy_partial();
    if (error)
      *error = "vkAllocateMemory failed for a facet image";
    return false;
  }
  if (vkBindImageMemory(device_, record.image, record.memory, 0) !=
      VK_SUCCESS) {
    destroy_partial();
    if (error)
  ...
  const VkImageAspectFlags aspect = format == VK_FORMAT_D32_SFLOAT
  ...
  if (vkCreateImageView(device_, &view_info, nullptr, &record.view) !=
      VK_SUCCESS) {
    destroy_partial();
    if (error)
      *error = "vkCreateImageView failed for a facet image";
    return false;
  }
  ...
  record.content_epoch = backing != nullptr ? backing->content_epoch : 0;
  record.kind = slot->kind;
  record.backing_bytes = view_bytes;
  record.allocation_padding_bytes =
      requirements.size > view_bytes
          ? static_cast<uint64_t>(requirements.size) - view_bytes
          : 0;
  ...
  if (source == VK_NULL_HANDLE) {
    const vg::core::Allocation *allocation = backing;
    if (allocation == nullptr) {
      destroy_partial();
      if (error)
        *error = "facet backing allocation is not active in this Arena";
      return false;
    }
    if (allocation->bytes.size() < view_bytes) {
      destroy_partial();
      if (error)
        *error = "the linear backing is smaller than the CanonicalView it "
                 "names, so the view's "
                 "subresource layout cannot be read out of it";
      return false;
    }
    if (!create_raw_buffer(device_, physical_device_, view_bytes,
                           VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                           /*want_address=*/false, /*want_map=*/true, &staging,
                           error)) {
      destroy_partial();
      return false;
    }
    std::memcpy(staging.mapped, allocation->bytes.data(),
                static_cast<size_t>(view_bytes));
    source = staging.buffer;
    source_offset = 0;
    if (temporary_bytes != nullptr)
      *temporary_bytes = view_bytes;
  }
  ...
  for (uint32_t layer = 0; layer < view.array_layers; ++layer) {
    for (uint32_t level = 0; level < view.mip_levels; ++level) {
      ...
      region.bufferOffset =
          source_offset + view.subresource_byte_offset({layer, level});
  ...
  record_layout_transition(command_buffer, &record,
                           VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
  vkCmdCopyBufferToImage(command_buffer, source, record.image,
                         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                         static_cast<uint32_t>(regions.size()), regions.data());
  record_layout_transition(command_buffer, &record,
                           facet_read_layout(expected_kind, format));
  vkEndCommandBuffer(command_buffer);
  if (!submit_and_wait_simple(device_, compute_queue_, command_pool_,
...
uint32_t
DeviceState::retire_stale_facet_images(const vg::core::Arena &arena,
                                       const vg::core::FacetPool &pool) {
  uint32_t retired = 0;
  for (auto it = facet_images_.begin(); it != facet_images_.end();) {
    const vg::core::FacetRef ref{it->second.facet_index,
                                 it->second.facet_generation};
...
bool DeviceState::ensure_sampler(vg::core::FilterMode filter,
                                 vg::core::WrapMode wrap, VkSampler *out,
                                 std::string *error) {
  const uint32_t key =
      (static_cast<uint32_t>(filter) << 1) | static_cast<uint32_t>(wrap);
  const auto cached = sampler_cache_.find(key);
  if (cached != sampler_cache_.end()) {
    *out = cached->second;
    return true;
  }
  const VkFilter vk_filter = filter == vg::core::FilterMode::Bilinear
                                 ? VK_FILTER_LINEAR
                                 : VK_FILTER_NEAREST;
  const VkSamplerAddressMode address_mode =
      wrap == vg::core::WrapMode::Repeat
  ...
  sampler_info.mipmapMode = filter == vg::core::FilterMode::Bilinear
  ...
  if (vkCreateSampler(device_, &sampler_info, nullptr, &sampler) !=
      VK_SUCCESS) {
    if (error)
...
bool DeviceState::ensure_descriptor_pool(std::string *error) {
  ...
  pool_info.poolSizeCount =
      static_cast<uint32_t>(sizeof(sizes) / sizeof(sizes[0]));
...
bool DeviceState::transform_representation(
  ...
  if (!facet_pool().acquire(arena, request.view, vg::core::FacetKind::Transfer,
                            &transfer_ref, error))
    return false;
  ...
  bool ok = false;
  {
    ...
    if (use.begin(arena, error)) {
      const vg::core::FacetSlot *transfer_slot =
          resolve_facet(arena, facet_pool(), transfer_ref,
                        vg::core::FacetKind::Transfer, error);
      const vg::core::Allocation *allocation =
          transfer_slot == nullptr
              ? nullptr
              : arena.lookup(core::PointerRef{
                    transfer_slot->view.allocation,
                    transfer_slot->view.allocation_generation});
      if (transfer_slot != nullptr && allocation == nullptr && error != nullptr)
        *error =
            "the transform's source allocation is not active in this Arena";
      if (allocation != nullptr) {
        ...
        if (ensure_buffer(*allocation, &source, error) &&
            ensure_facet_image(arena, facet_pool(), facet, request.target_kind,
                               source->buffer,
                               /*upload_source_offset=*/0, &image, &cache_hit,
                               &temporary_bytes, error)) {
...
DeviceState::~DeviceState() {
  ...
  if (device_ != VK_NULL_HANDLE) {
    destroy_plan_raster_cache(*this);
    destroy_plan_indirect_cache(*this);
    ...
    if (task_ring_pipeline_ != VK_NULL_HANDLE)
      vkDestroyPipeline(device_, task_ring_pipeline_, nullptr);
    ...
    if (raster_vertex_module_ != VK_NULL_HANDLE)
      vkDestroyShaderModule(device_, raster_vertex_module_, nullptr);
