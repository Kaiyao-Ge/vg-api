...
void write(const Value& value,std::string& out){ if(std::holds_alternative<std::nullptr_t>(value.data)){out+="null";}else if(const auto* b=std::get_if<bool>(&value.data)){out+=*b?"true":"false";}else if(const auto* i=std::get_if<int64_t>(&value.data)){out+=std::to_string(*i);}else if(const auto* s=std::get_if<std::string>(&value.data)){append_escaped(*s,out);}else if(const auto* a=std::get_if<Value::Array>(&value.data)){out+='[';for(size_t i=0;i<a->size();++i){if(i)out+=',';write((*a)[i],out);}out+=']';}else{out+='{';bool first=true;for(const auto& [key,child]:std::get<Value::Object>(value.data)){if(!first)out+=',';first=false;append_escaped(key,out);out+=':';write(child,out);}out+='}';}}
}
